...
# Load results
with open(os.path.join(results_path, "baseline_ppo_results_for_comparison.pkl"), "rb") as f:
    ppo_results = pickle.load(f)
...
# Load results
with open(os.path.join(results_path, "ppo_results_for_comparison.pkl"), "rb") as f:
    ppo_results = pickle.load(f)
...
plot_curves([np.array(baseline_ppo_returns), np.array(ppo_returns)], ['Baseline PPO', 'PPO'], ['b', 'r'], 'Return',
            'Baseline PPO vs PPO Returns', smoothing=True)
